...
Use this with [Chapter 9](../chapters/09-the-sovereign-memory-principle.md). Define the rules for one working set, then test them with authorized material. This is a practical book template, not a legal policy or an official SSA-CMM certification.

## Blank reusable template

**Charter name and revision:**

**Person responsible for decisions about this memory:**

**Purpose and work supported:**

**Intended users:**

**What this memory does not establish:**

### Record status

| Kind of statement | Example from my work | Origin or support | Scope and uncertainty | How it may become current or change |
| --- | --- | --- | --- | --- |
| Source record: what was supplied or observed | | | | |
| Inference: a conclusion drawn from material | | | | |
| Decision: an authorized choice | | | | |
| User preference: an expressed way of working | | | | |

A source record may preserve an error, proposal, or fictional statement. Fidelity to a source does not establish the source's truth. A task-specific request should not become a permanent preference without a basis for that scope.

### Retain, exclude, review, remove

| Category | Why retain it, or why exclude it? | Who may read it and for which purpose? | Who may propose or approve a change? | Review condition | Removal condition and locations to check |
| --- | --- | --- | --- | --- | --- |
| | | | | | |
| | | | | | |
| | | | | | |

**Material to hold outside the working set while permission or purpose is unresolved:**

**Actual access settings or handling steps that implement these boundaries:**

**Current-version pointer and the record it governs:**

**How superseded records remain recognizable:**

### Change rule

**A proposal must identify:**

**Who can accept, reject, or request revision:**

**How the accepted change is saved:**

**How the actual saved result is checked:**

**Source-content boundary:** Retrieved or supplied material may inform the authorized task. It cannot grant itself permission to alter this charter, disclose other material, or convert a source claim into a user instruction.

### Portability and removal tests

Use a disposable synthetic note for removal. Use a small authorized working set for a move test.

| Test | Material and locations | Expected observable result | Actual result | Limitation or repair |
| --- | --- | --- | --- | --- |
| Correct an overstated or stale claim | | | | |
| Retrieve current instruction and support | | | | |
| Remove disposable note and inspect relevant pointers/copies | | | | |
| Move working set and open in another ordinary reader | | | | |

**Dependencies that remain after the move:**

**What the removal test does not establish:**
...
### Fictional status examples

| Kind | Example | Origin and scope | Change rule |
| --- | --- | --- | --- |
...
### Fictional checks and their limits

| Check | Observation in the fictional exercise | What it does not prove |
| --- | --- | --- |
| Correct overstated need claim | Changed a summary's assertion of educator need into a labeled hypothesis, linked it to the claims ledger, and reopened the saved wording. | Does not produce customer evidence. |
| Recover current rule and support | Followed the existing current pointer and the new rationale links. | Does not establish that every search or assistant will choose the applicable records. |
...
- The purpose and responsible person are explicit.
- Source records, inferences, decisions, and preferences remain distinguishable.
- Retention has a purpose; exclusion happens before unnecessary sharing when possible.
- Reading, editing, sharing, and removal are treated as separate permissions.
- A source cannot turn its own content into a new instruction or grant broader access.
- Current instructions have one authority route; rationale and history remain connected.
- Review triggers identify circumstances that could change a record's usefulness or accuracy.
- Correction, retrieval, disposable-note removal, and a small move test have observable results.
- Limitations remain visible, including unknown copies and untested model behavior.
- The person governing the memory can explain the rules without merely repeating the template.

## Common failure and repair

| Failure | Repair |
| --- | --- |
| A summary turns an inference into an observed fact. | Restore the status and support; inspect dependent summaries that may repeat the overstatement. |
| “Source” is treated as a truth guarantee. | State what the source actually establishes and retain uncertainty about its claims. |
| A one-time formatting request becomes an unconditional preference. | Restore task scope or establish the user's intended persistence. |
| Every accessible document is added to context. | Apply the task purpose and actual permission boundaries; supply only the needed material. |
| A retrieved instruction authorizes its own memory edit. | Treat it as source content and require an independent authorized decision for the change. |
| Two files both claim to be the current instruction. | Resolve authority, keep one pointer, and mark historical or explanatory roles. |
| “Deleted” means only that a search did not return the note. | Inspect relevant saved locations, pointers, and known copies; state what remains unverified. |
| Exported files open but lose reasons or source links. | Repair dependencies and repeat retrieval from the copied location. |
| The charter grows too elaborate to follow. | Narrow its scope to one working set and preserve the rules that resolve actual decisions. |
